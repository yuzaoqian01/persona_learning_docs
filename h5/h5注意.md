...
有些`移动端浏览器`会自动将数字字母[符号识别](https://zhida.zhihu.com/search?content_id=166767904&content_type=Article&match_order=1&q=符号识别&zd_token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJ6aGlkYV9zZXJ2ZXIiLCJleHAiOjE3NjQzODU4NTgsInEiOiLnrKblj7for4bliKsiLCJ6aGlkYV9zb3VyY2UiOiJlbnRpdHkiLCJjb250ZW50X2lkIjoxNjY3Njc5MDQsImNvbnRlbnRfdHlwZSI6IkFydGljbGUiLCJtYXRjaF9vcmRlciI6MSwiemRfdG9rZW4iOm51bGx9.YKgA8vdIjpyszQWh7zbmqK_CBCDNB-yhhjX4pVTT-f4&zhida_source=entity)为`电话/邮箱`并将其渲染成上述**调用系统功能**里的`<a>`。虽然很方便却有可能违背需求。
...
通过`location.href`与原生应用建立通讯渠道，这种页面与客户端的通讯方式称为**URL Scheme**，其基本格式为`scheme://[path][?query]`，笔者曾经发表过[《H5与App的通讯方式》](https://link.zhihu.com/?target=https%3A//juejin.cn/post/6844904020201455624)讲述`URL Scheme`的使用。
...
### 禁止[页面缓存](https://zhida.zhihu.com/search?content_id=166767904&content_type=Article&match_order=1&q=页面缓存&zd_token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJ6aGlkYV9zZXJ2ZXIiLCJleHAiOjE3NjQzODU4NTgsInEiOiLpobXpnaLnvJPlrZgiLCJ6aGlkYV9zb3VyY2UiOiJlbnRpdHkiLCJjb250ZW50X2lkIjoxNjY3Njc5MDQsImNvbnRlbnRfdHlwZSI6IkFydGljbGUiLCJtYXRjaF9vcmRlciI6MSwiemRfdG9rZW4iOm51bGx9.Kjy6rOKwPCWDhIhw7HJfwzd0t5BWbuORpwa-BNm_0RQ&zhida_source=entity)

**Cache-Control**指定请求和响应遵循的[缓存机制](https://zhida.zhihu.com/search?content_id=166767904&content_type=Article&match_order=1&q=缓存机制&zd_token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJ6aGlkYV9zZXJ2ZXIiLCJleHAiOjE3NjQzODU4NTgsInEiOiLnvJPlrZjmnLrliLYiLCJ6aGlkYV9zb3VyY2UiOiJlbnRpdHkiLCJjb250ZW50X2lkIjoxNjY3Njc5MDQsImNvbnRlbnRfdHlwZSI6IkFydGljbGUiLCJtYXRjaF9vcmRlciI6MSwiemRfdG9rZW4iOm51bGx9.0vMboXndhMwirn2NuGhqKaKesdkiCvbndx9xANc1cqQ&zhida_source=entity)，不想使用浏览器缓存就禁止呗！
...
针对移动端，笔者通常会结合JS依据`屏幕宽度`与`设计图宽度`的比例动态声明`<html>`的`font-size`，以`rem`为长度单位声明所有节点的[几何属性](https://zhida.zhihu.com/search?content_id=166767904&content_type=Article&match_order=1&q=几何属性&zd_token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJ6aGlkYV9zZXJ2ZXIiLCJleHAiOjE3NjQzODU4NTgsInEiOiLlh6DkvZXlsZ7mgKciLCJ6aGlkYV9zb3VyY2UiOiJlbnRpdHkiLCJjb250ZW50X2lkIjoxNjY3Njc5MDQsImNvbnRlbnRfdHlwZSI6IkFydGljbGUiLCJtYXRjaF9vcmRlciI6MSwiemRfdG9rZW4iOm51bGx9.eVDufXjuJOoNo3APqIpTqDsRbBRxE3Ym4XkNUZPXKA4&zhida_source=entity)，这样就能做到大部分移动设备的页面兼容，兼容出入较大的地方再通过`媒体查询`做特别处理。
...
对于一些突然出现滚动条的页面，可能会产生左右抖动的不良影响。在一个滚动容器里，打开弹窗就隐藏滚动条，关闭弹窗就显示滚动条，来回操作会让屏幕抖动起来。提前声明[滚动容器](https://zhida.zhihu.com/search?content_id=166767904&content_type=Article&match_order=2&q=滚动容器&zd_token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJ6aGlkYV9zZXJ2ZXIiLCJleHAiOjE3NjQzODU4NTgsInEiOiLmu5rliqjlrrnlmagiLCJ6aGlkYV9zb3VyY2UiOiJlbnRpdHkiLCJjb250ZW50X2lkIjoxNjY3Njc5MDQsImNvbnRlbnRfdHlwZSI6IkFydGljbGUiLCJtYXRjaF9vcmRlciI6MiwiemRfdG9rZW4iOm51bGx9.8JfmrZRUmCd7d2Hthwsd1v1M4FLkvw-IdYBetiueQxY&zhida_source=entity)的`padding-right`为滚动条宽度，就能有效消除这个不良影响。
...
[fastclick](https://link.zhihu.com/?target=https%3A//github.com/ftlabs/fastclick)有现成的`NPM包`，可直接安装到项目里。引入`fastclick`可使用`click事件`代替`tap事件`，接入方式极其简单。
...
该解决方案在视觉上无任何变化，完爆其他解决方案，其实就是一种反向思维和[障眼法](https://zhida.zhihu.com/search?content_id=166767904&content_type=Article&match_order=1&q=障眼法&zd_token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJ6aGlkYV9zZXJ2ZXIiLCJleHAiOjE3NjQzODU4NTgsInEiOiLpmpznnLzms5UiLCJ6aGlkYV9zb3VyY2UiOiJlbnRpdHkiLCJjb250ZW50X2lkIjoxNjY3Njc5MDQsImNvbnRlbnRfdHlwZSI6IkFydGljbGUiLCJtYXRjaF9vcmRlciI6MSwiemRfdG9rZW4iOm51bGx9.Vd5aJ2C90MQp377bqY5H-VVxNmcd1B6CSV_X44AjttU&zhida_source=entity)。该解决方案完美解决`固定弹窗`和`滚动弹窗`对`<body>`全局滚动的影响，当然也可用于局部滚动容器里，因此很值得推广。
...
只要保持前后滚动条偏移量一致就不会出现上述问题。在输入框聚焦时获取页面当前滚动条偏移量，在输入框失焦时赋值页面之前获取的滚动条偏移量，这样就能[间接还原](https://zhida.zhihu.com/search?content_id=166767904&content_type=Article&match_order=1&q=间接还原&zd_token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJ6aGlkYV9zZXJ2ZXIiLCJleHAiOjE3NjQzODU4NTgsInEiOiLpl7TmjqXov5jljp8iLCJ6aGlkYV9zb3VyY2UiOiJlbnRpdHkiLCJjb250ZW50X2lkIjoxNjY3Njc5MDQsImNvbnRlbnRfdHlwZSI6IkFydGljbGUiLCJtYXRjaF9vcmRlciI6MSwiemRfdG9rZW4iOm51bGx9.2UdiERsgFVCYiI1_ZqMkkzr__WT5SEEcJF3Hpp9Dps4&zhida_source=entity)页面滚动条偏移量解决页面高度坍塌。
...
该函数就是[scrollIntoView](https://link.zhihu.com/?target=https%3A//developer.mozilla.org/zh-cn/docs/web/api/element/scrollintoview)，它会滚动[目标元素](https://zhida.zhihu.com/search?content_id=166767904&content_type=Article&match_order=1&q=目标元素&zd_token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJ6aGlkYV9zZXJ2ZXIiLCJleHAiOjE3NjQzODU4NTgsInEiOiLnm67moIflhYPntKAiLCJ6aGlkYV9zb3VyY2UiOiJlbnRpdHkiLCJjb250ZW50X2lkIjoxNjY3Njc5MDQsImNvbnRlbnRfdHlwZSI6IkFydGljbGUiLCJtYXRjaF9vcmRlciI6MSwiemRfdG9rZW4iOm51bGx9.6yTiVoBm39mJz86-EAgVz44kRkBv7WSAHtjFVvbsr_U&zhida_source=entity)的父容器使之对用户可见，简单概括就是相对视窗让容器滚动到目标元素位置。它有三个可选参数能让`scrollIntoView`滚动起来更优雅。
...
该函数就是[IntersectionObserver](https://link.zhihu.com/?target=https%3A//developer.mozilla.org/zh-CN/docs/Web/API/IntersectionObserver)，它提供一种异步观察目标元素及其祖先元素或顶级文档视窗交叉状态的方法。详情可参照[MDN文档](https://link.zhihu.com/?target=https%3A//developer.mozilla.org/zh-CN/docs/Web/API/IntersectionObserver)，在此不作过多介绍。
